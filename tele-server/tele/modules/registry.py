...
from tele.tools import (config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, sql,
                        Collector, OrderedSet)
from tele.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('tele.schema')
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # tele.service.wsgi_server.application
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        with cls._lock:
            ...
            try:
                ...
                try:
                    tele.modules.load_modules(registry, force_demo, status, update_module)
                except Exception:
                    tele.modules.reset_modules_state(db_name)
                    raise
            except Exception:
    ...
    def init(self, db_name):
        ...
        self._init = True
        self._assertion_report = tele.tests.runner.TeleTestResult()
        ...
        self._db = tele.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = tele.modules.db.has_unaccent(cr)
            self.has_trigram = tele.modules.db.has_trigram(cr)
    ...
    def setup_models(self, cr):
        ...
        """
        env = tele.api.Environment(cr, SUPERUSER_ID, {})
        env['base'].flush()

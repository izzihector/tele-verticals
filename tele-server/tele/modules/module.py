...
import tele
import tele.tools as tools
import tele.release as release
from tele.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__telecms__.py')
README = ['README.rst', 'README.md', 'README.txt']

_logger = logging.getLogger(__name__)

# applets path as a list
# ad_paths is a deprecated alias, please use tele.applets.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"tele.modules.module.ad_paths" is a deprecated proxy to '
        '"tele.applets.__path__".', DeprecationWarning, stacklevel=2)
    return tele.applets.__path__

# Modules already loaded
loaded = []

class AppletsHook(object):
    """ Makes modules accessible through telecms.applets.* """

    def find_module(self, name, path=None):
        if name.startswith('telecms.applets.') and name.count('.') == 2:
            warnings.warn(
                '"telecms.applets" is a deprecated alias to "tele.applets".',
    ...
    def load_module(self, name):
        ...
        tele_name = re.sub(r'^telecms.applets.(\w+)$', r'tele.applets.\g<1>', name)

        tele_module = sys.modules.get(tele_name)
        if not tele_module:
            tele_module = importlib.import_module(tele_name)

        sys.modules[name] = tele_module

        return tele_module

class TeleHook(object):
    """ Makes tele package also available as telecms """

    def find_module(self, name, path=None):
        # telecms.applets.<identifier> should already be matched by AppletsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^telecms\b', name):
            warnings.warn(
                'telecms is a deprecated alias to tele.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^telecms(.*)', r'tele\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `tele.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^tele.applets.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("tele.applets.base.maintenance.migrations", "tele.upgrade")
...
def initialize_sys_path():
    """
    Setup the applets path ``tele.applets.__path__`` with various defaults
    and explicit directories.
    """
    # hook tele.applets on data dir
    dd = os.path.normcase(tools.config.applets_data_dir)
    if os.access(dd, os.R_OK) and dd not in tele.applets.__path__:
        tele.applets.__path__.append(dd)

    # hook tele.applets on applets paths
    for ad in tools.config['applets_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in tele.applets.__path__:
            tele.applets.__path__.append(ad)

    # hook tele.applets on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'applets')))
    if base_path not in tele.applets.__path__ and os.path.isdir(base_path):
        tele.applets.__path__.append(base_path)

    # hook tele.upgrade on upgrade-path
    from tele import upgrade
    ...
    # create decrecated module alias from tele.applets.base.maintenance.migrations to tele.upgrade
    spec = importlib.machinery.ModuleSpec("tele.applets.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["tele.applets.base.maintenance"] = maintenance_pkg
    sys.modules["tele.applets.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from telecms to tele and "crm"-like to tele.applets
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    Search the applets paths and return the first path where the given
    module is found. If downloaded is True, return the default applets
    path if nothing else is found.

    """
    for adp in tele.applets.__path__:
    ...
    if downloaded:
        return opj(tools.config.applets_data_dir, module)
    if display_warning:
...
def get_module_filetree(module, dir='.'):
    ...
    files = tele.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in tele.applets.__path__:
...
def read_manifest(applets_path, module):
...
def load_telecms_module(module_name):
    """ Load an Tele module, if not already loaded.
    ...
    try:
        __import__('tele.applets.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['tele.applets.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in tele.applets.__path__:
        if not os.path.exists(ad):
            _logger.warning("applets path does not exist: %s", ad)
            continue

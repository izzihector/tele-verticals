...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('tele.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='tele')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='tele.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running Tele
...
DEFAULT_LOG_CONFIGURATION = [
    'tele.http.rpc.request:INFO',
    'tele.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['tele:DEBUG', 'tele.sql_db:INFO', 'tele.http.rpc:DEBUG'],
    'debug_rpc': ['tele:DEBUG', 'tele.sql_db:INFO', 'tele.http.rpc.request:DEBUG'],
    'debug': ['tele:DEBUG', 'tele.sql_db:INFO'],
    'debug_sql': ['tele.sql_db:DEBUG'],
    'info': [],
    'runbot': ['tele:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['tele:WARNING', 'werkzeug:WARNING'],
    'error': ['tele:ERROR', 'werkzeug:ERROR'],
    'critical': ['tele:CRITICAL', 'werkzeug:CRITICAL'],
}

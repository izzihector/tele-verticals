...
from tele.exceptions import UserError, AccessError
from tele.tools.safe_eval import safe_eval, time
from tele.tools.misc import find_in_path
from tele.tools import config, is_html_empty, parse_version
from tele.sql_db import TestCursor
from tele.http import request
from tele.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Tele so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Tele start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Tele with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "Tele is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"

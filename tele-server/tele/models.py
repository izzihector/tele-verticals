    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('tele.applets.'), \
                    f"Invalid import of {module}.{name}, it should start with 'tele.applets'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Tele models.

    Tele models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~tele.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~tele.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~tele.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the Tele sense)
    ...
    .. warning::
      ...
      :attr:`~tele.models.Model._inherits`-ed models, the inherited field will
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = tele.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], tele.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], tele.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from tele.fields import Char, Integer
    ...
    def get_formview_id(self, access_uid=None):
        """ Return an view id to open the document ``self`` with. This method is
            meant to be overridden in applets that want to give specific view ids
            for example.
    ...
    def get_formview_action(self, access_uid=None):
        """ Return an action to open the document ``self``. This method is meant
            to be overridden in applets that want to give specific view ids for
            example.
    ...
    def get_access_action(self, access_uid=None):
        ...
        overridden in applets that want to give specific access to the document.
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(tele.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(tele.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~tele.applets.base.models.res_company`).

        For :class:`~tele.applets.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from tele.applets.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~tele.fields.Integer`,
          :class:`~tele.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~tele.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~tele.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~tele.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~tele.fields.Date` and
              :class:`~tele.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`tele.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`tele.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _telecms/models/relationals/format:

          The expected value of a :class:`~tele.fields.One2many` or
          :class:`~tele.fields.Many2many` relational field is a list of
          :class:`~tele.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~tele.fields.Command.create`,
          :meth:`~tele.fields.Command.update`,
          :meth:`~tele.fields.Command.delete`,
          :meth:`~tele.fields.Command.unlink`,
          :meth:`~tele.fields.Command.link`,
          :meth:`~tele.fields.Command.clear`, and
          :meth:`~tele.fields.Command.set`.
          """
        if not self:
            return True
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an Tele domain.
    ...
    def ensure_one(self):
        ...
        :raise tele.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~tele.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~tele.applets.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/tele/master-nochange-fp/tele/applets/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`tele.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Tele models.

    Tele models are created by inheriting from this class::

...
Tele domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an Tele domain expression from an untrusted source.
    ...
    except tele.exceptions.UserError:
        raise
    except tele.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except tele.http.AuthenticationError:
        raise
...
`tele.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `tele.tools.safe_eval`.
""")
    return d

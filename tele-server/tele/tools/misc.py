...
"""
Miscellaneous tools used by Tele.
"""
import cProfile
import collections
import datetime
...
from tele.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if tele.tools.config['db_host']:
        env['PGHOST'] = tele.tools.config['db_host']
    if tele.tools.config['db_port']:
        env['PGPORT'] = str(tele.tools.config['db_port'])
    if tele.tools.config['db_user']:
        env['PGUSER'] = tele.tools.config['db_user']
    if tele.tools.config['db_password']:
        env['PGPASSWORD'] = tele.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    """Verify that a file exists under a known `applets_path` directory and return its full path.
    ...
    :param str file_path: absolute file path, or relative path within any `applets_path` directory
    :param list[str] filter_ext: optional list of supported extensions (lowercase, with leading dot)
    :return: the absolute path to the file
    :raise FileNotFoundError: if the file is not found under the known `applets_path` directories
    :raise ValueError: if the file doesn't have one of the supported extensions (`filter_ext`)
    """
    root_path = os.path.abspath(config['root_path'])
    applets_paths = tele.applets.__path__ + [root_path]
    ...
    # ignore leading 'applets/' if present, it's the final component of root_path, but
    # may sometimes be included in relative paths
    if normalized_path.startswith('applets' + os.sep):
        normalized_path = normalized_path[7:]

    for applets_dir in applets_paths:
        # final path sep required to avoid partial match
        parent_path = os.path.normpath(os.path.normcase(applets_dir)) + os.sep
...
def file_open(name, mode="r", filter_ext=None):
    """Open a file from within the applets_path directories, as an absolute or relative path.
...
def scan_languages():
    """ Returns all languages supported by Tele for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = tele.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating Tele domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('tele.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('tele.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = tele.fields.Datetime.from_string(value)
            value = tele.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = tele.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = tele.fields.Datetime.from_string(value)
    else:
        timestamp = value

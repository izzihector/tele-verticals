...
import { StudioNavbar } from "@tele_studio/client_action/navbar/navbar";
...
QUnit.module("Studio > Navbar", (hooks) => {
    ...
    QUnit.test("homemenu customizer rendering", async (assert) => {
        ...
        assert.containsOnce(navbar.el, ".o_tele_studio_home_studio_menu");

        await click(navbar.el.querySelector(".o_tele_studio_home_studio_menu .dropdown-toggle"));

        assert.containsOnce(navbar.el, ".o_tele_studio_change_background");
        assert.strictEqual(navbar.el.querySelector(".o_tele_studio_change_background input").accept, "image/*", "Input should now only accept images");

        assert.containsOnce(navbar.el, ".o_tele_studio_import");
        assert.containsOnce(navbar.el, ".o_tele_studio_export");
    });
});

...
const { Component } = owl;
const sampleIconUrl = "/tele_enterprise/Parent.src/img/default_icon_app.png";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("app creator: standard flow with model creation", async function (assert) {
        ...
        const { wrapper, appCreator } = await createAppCreator({
            ...
            rpc: async (route, params) => {
                if (route === "/tele_studio/create_new_app") {
        ...
        assert.containsNone(
            appCreator,
            ".o_tele_studio_app_creator_previous",
            "Previous button should not be rendered at step welcome"
        );
        assert.hasClass(
            appCreator.el.querySelector(".o_tele_studio_app_creator_next"),
            "is_ready",
            "Next button should be ready at step welcome"
        );

        // go to step: 'app'
        await testUtils.dom.click(appCreator.el.querySelector(".o_tele_studio_app_creator_next"));

        assert.strictEqual(appCreator.state.step, "app", "Current step should be app");
        assert.containsOnce(
            appCreator,
            ".o_tele_studio_icon_creator .o_tele_studio_selectors",
        ...
        await testUtils.dom.click(appCreator.el.getElementsByClassName("o_tele_studio_selector")[0]);

        assert.containsOnce(
            appCreator,
            ".o_tele_studio_palette",
            "the first palette should be open"
        );

        await testUtils.dom.triggerEvent(
            appCreator.el.querySelector(".o_tele_studio_palette"),
            "mouseleave"
        );
        ...
        await testUtils.dom.click(
            appCreator.el.querySelectorAll(".o_tele_studio_selectors > .o_tele_studio_selector")[0]
        );
        await testUtils.dom.click(
            appCreator.el.querySelectorAll(".o_tele_studio_selectors > .o_tele_studio_selector")[1]
        );
        ...
        await testUtils.dom.click(appCreator.el.querySelectorAll(".o_tele_studio_palette div")[2]);
        await testUtils.dom.click(
            appCreator.el.querySelectorAll(".o_tele_studio_selectors > .o_tele_studio_selector")[2]
        );
        await testUtils.dom.click(
            appCreator.el.querySelectorAll(".o_tele_studio_icons_library div")[43]
        );

        await testUtils.dom.triggerEvent(
            appCreator.el.querySelector(".o_tele_studio_icons_library"),
            "mouseleave"
        );
        ...
        assert.strictEqual(
            appCreator.el.querySelectorAll(".o_tele_studio_selector")[1].style.backgroundColor,
        ...
        assert.hasClass(
            appCreator.el.querySelector(".o_tele_studio_selector i"),
            "fa fa-heart",
        ...
        await testUtils.dom.triggerEvent(
            appCreator.el.querySelector(".o_tele_studio_upload input"),
            "change"
        );
        ...
        assert.strictEqual(
            appCreator.el.querySelector(".o_tele_studio_uploaded_image").style.backgroundImage,
        ...
        assert.hasClass(
            appNameInput,
            "o_tele_studio_app_creator_field_warning",
        ...
        await testUtils.dom.click(
            appCreator.el.querySelector(".o_tele_studio_model_configurator_previous")
        );
        await testUtils.dom.click(appCreator.el.querySelector(".o_tele_studio_app_creator_next"));
        ...
        await testUtils.dom.click(
            appCreator.el.querySelector(".o_tele_studio_model_configurator_next")
        );
    ...
    QUnit.test("app creator: has 'lines' options to auto-create a one2many", async function (assert) {
        ...
        assert.containsOnce(appCreator, ".o_tele_studio_model_configurator_option input[type='checkbox'][name='lines'][id='lines']");
        assert.strictEqual(
            appCreator.el.querySelector("label[for='lines']").textContent,
            "LinesAdd details to your records with an embedded list view"
        );

        await testUtils.dom.click(appCreator.el.querySelector(".o_tele_studio_model_configurator_option input[type='checkbox'][name='lines']"));
        await testUtils.dom.click(appCreator.el.querySelector(".o_tele_studio_model_configurator_next"));
        wrapper.destroy();
    });
    ...
    QUnit.test("app creator: debug flow with existing model", async function (assert) {
            ...
            async rpc(route, params) {
                ...
                switch (route) {
                    case "/web/dataset/call_kw/ir.model/name_search":
                        ...
                        return [[69, "The Value"]];
                    case "/tele_studio/create_new_app":
                        assert.strictEqual(
                            params.model_id,
                            69,
        ...
        let buttonNext = appCreator.el.querySelector("button.o_tele_studio_app_creator_next");
        ...
        buttonNext = appCreator.el.querySelector("button.o_tele_studio_app_creator_next");
        ...
        assert.doesNotHaveClass(
            appCreator.el.querySelector(".o_tele_studio_app_creator_model"),
            "o_tele_studio_app_creator_field_warning"
        );

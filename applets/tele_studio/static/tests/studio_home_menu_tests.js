...
import { StudioHomeMenu } from "@tele_studio/client_action/studio_home_menu/studio_home_menu";
import { MODES } from "@tele_studio/studio_service";

import { makeFakeEnterpriseService } from "@tele_enterprise/../tests/mocks";
...
QUnit.module("Studio", (hooks) => {
            ...
            apps: [
                {
                    ...
                    webIconData: "/tele_enterprise/static/img/default_icon_app.png",
                    xmlid: "app.1",
                },
                {
                    actionID: 122,
    ...
    QUnit.test("simple rendering", async function (assert) {
        ...
        assert.strictEqual(
            ...
            'url("/tele_enterprise/static/img/default_icon_app.png")'
        );
        assert.containsOnce(firstApp, "div.o_caption");
        assert.strictEqual(firstApp.querySelector("div.o_caption").innerText, "Connect");
        assert.containsOnce(firstApp, ".o_tele_studio_edit_icon i");
        ...
        assert.containsOnce(secondApp, ".o_tele_studio_edit_icon i");

        // New app button
        assert.containsOnce(
            studioHomeMenu.el,
            "div.o_apps > a.o_app.o_tele_studio_new_app",
            'should contain a "New App icon"'
        );
        const newApp = studioHomeMenu.el.querySelector("a.o_app.o_tele_studio_new_app");
        assert.strictEqual(
            newApp.querySelector("div.o_app_icon").style.backgroundImage,
            'url("/tele_studio/static/src/img/default_icon_app.png")',
            "Image source URL should end with '/tele_studio/static/src/img/default_icon_app.png'"
        );
    ...
    QUnit.test("Click on new App", async function (assert) {
        ...
        await testUtils.dom.click(studioHomeMenu.el.querySelector("a.o_app.o_tele_studio_new_app"));
    ...
    QUnit.test("Click on edit icon button", async function (assert) {
        ...
        const firstEditIconButton = studioHomeMenu.el.querySelector(".o_tele_studio_edit_icon i");
        ...
        assert.containsOnce(
            dialog,
            ".modal-content.o_tele_studio_edit_menu_icon_modal .o_tele_studio_icon_creator"
        );
    ...
    QUnit.test("edit an icon", async function (assert) {
        ...
        const mockRPC = (route, args) => {
            if (route === "/tele_studio/edit_menu_icon") {
        ...
        await testUtils.dom.click(studioHomeMenu.el.querySelector(".o_tele_studio_edit_icon i"));
        const dialog = document.querySelector("div.modal");
        await testUtils.dom.click(dialog.querySelector(".o_tele_studio_upload a"));

        assert.doesNotHaveClass(
            dialog.querySelector(".o_tele_studio_icon .o_app_icon i"),
            "fa-balance-scale"
        );

        // Change the icon's pictogram
        await testUtils.dom.click(dialog.querySelectorAll(".o_tele_studio_selector")[2]);
        await testUtils.dom.click(
            dialog.querySelector(".o_tele_studio_selector .fa.fa-balance-scale")
        );

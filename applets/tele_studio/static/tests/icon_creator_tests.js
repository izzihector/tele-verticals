...
import { IconCreator } from "@tele_studio/client_action/icon_creator/icon_creator";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("icon creator: with initial web icon data", async function (assert) {
        ...
        assert.strictEqual(
            parent.el.querySelector(".o_tele_studio_uploaded_image").style.backgroundImage,
            `url(\"${sampleIconUrl}\")`,
            "displayed image should prioritize web icon data"
        );

        // click on first link: "Design icon"
        await testUtils.dom.click(parent.el.querySelector(".o_tele_studio_upload a"));

        assert.verifySteps(["icon-changed"]);
        assert.strictEqual(parent.el.querySelector('.o_tele_studio_upload input').accept, 'image/png', "Input should now only accept pngs");

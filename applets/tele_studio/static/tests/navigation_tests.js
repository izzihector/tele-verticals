...
import { createEnterpriseWebClient } from "@tele_enterprise/../tests/helpers";
import testUtils from "web.test_utils";
import { openStudio, leaveStudio, registerStudioDependencies } from "@tele_studio/../tests/helpers";
...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("Studio not available for non system users", async function (assert) {
        ...
        assert.containsNone(webClient, ".o_main_navbar .o_tele_studio_navbar_item a");
    });
    ...
    QUnit.test("open Studio with act_window", async function (assert) {
        ...
        assert.containsOnce(webClient, ".o_main_navbar .o_tele_studio_navbar_item a");

        await openStudio(webClient);

        assert.verifySteps(
            [
                "/tele_studio/activity_allowed",
                "/tele_studio/get_studio_view_arch",
        ...
        assert.containsOnce(
            webClient,
            ".o_tele_studio_client_action .o_tele_studio_kanban_view_editor",
        ...
        assert.containsOnce(webClient, ".o_studio_navbar .o_tele_studio_leave a");
        ...
        assert.containsNone(webClient, ".o_tele_studio_client_action", "Studio should be closed");
        assert.containsOnce(
            webClient,
    ...
    QUnit.test("open Studio with act_window and viewType", async function (assert) {
        ...
        assert.containsOnce(
            webClient,
            ".o_tele_studio_client_action .o_tele_studio_form_view_editor",
    ...
    QUnit.test("switch view and close Studio", async function (assert) {
        ...
        // click on tab "Views"
        await testUtils.dom.click(
            webClient.el.querySelector(".o_tele_studio_menu .o_tele_studio_menu_item a")
        );
        assert.containsOnce(webClient, ".o_tele_studio_action_editor");

        // open list view
        await testUtils.dom.click(
            webClient.el.querySelector(
                ".o_tele_studio_views .o_tele_studio_view_type[data-type=list] .o_tele_studio_thumbnail"
            )
        );
        assert.containsOnce(
            webClient,
            ".o_tele_studio_client_action .o_tele_studio_list_view_editor"
        );
    ...
    QUnit.test("keep action context when leaving Studio", async function (assert) {
        ...
        assert.containsOnce(webClient, ".o_tele_studio_kanban_view_editor");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await testUtils.dom.click(
                webClient.el.querySelector(".o_tele_studio_form_view_editor .o_field_one2many")
            );
            await testUtils.dom.click(
                webClient.el.querySelector(
                    '.o_tele_studio_form_view_editor .o_field_one2many .o_tele_studio_editX2Many[data-type="form"]'
                )
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            webClient.env.bus.on("ACTION_MANAGER:UI-UPDATED", null, () => {
                assert.containsOnce(webClient, ".o_kanban_view");
                assert.hasClass(
                    webClient.el.querySelector(".o_tele_studio_navbar_item"),
                    "o_disabled"
                );
                def.resolve();
            });
            ...
            assert.containsOnce(webClient, ".o_kanban_view");
            assert.hasClass(webClient.el.querySelector(".o_tele_studio_navbar_item"), "o_disabled");
        }
    );
});

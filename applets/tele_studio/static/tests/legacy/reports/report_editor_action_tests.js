tele.define('tele_studio.ReportEditorAction_tests', function (require) {
    "use strict";

    const { controlPanel } = require('web.test_utils');
    const { getPagerValue, pagerNext } = controlPanel;

    const { doActionAndOpenStudio, registerStudioDependencies,getReportServerData } = require("@tele_studio/../tests/helpers");
    ...
    QUnit.module('Studio', {
        ...
        QUnit.test('use pager', async function (assert) {
            ...
            const mockRPC = (route, args) => {
                switch (route) {
                    case "/tele_studio/get_report_views":
                        return { report_html: reportHTML };
                    case "/tele_studio/get_widgets_available_options":
                    case "/tele_studio/read_paperformat":
                        return {};
                }
            };

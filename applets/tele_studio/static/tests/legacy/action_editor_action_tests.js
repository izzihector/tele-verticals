tele.define('tele_studio.ActionEditorActionTests', function (require) {
    "use strict";
    ...
    QUnit.module('Studio', {
        ...
        QUnit.test('add a gantt view', async function (assert) {
            ...
            const mockRPC = (route, args) => {
                if (route === '/tele_studio/add_view_type') {
            ...
            await testUtils.dom.click($(webClient.el).find('.o_tele_studio_view_type[data-type="gantt"] .o_tele_studio_thumbnail'));
            await legacyExtraNextTick();

            assert.containsOnce($, '.o_tele_studio_new_view_dialog',
                "there should be an opened dialog to select gantt attributes");
            assert.strictEqual($('.o_tele_studio_new_view_dialog select[name="date_start"]').val(), 'start',
                "date start should be prefilled (mandatory)");
            assert.strictEqual($('.o_tele_studio_new_view_dialog select[name="date_stop"]').val(), 'start',
        ...
        QUnit.test('disable the view from studio', async function (assert) {
            ...
            const mockRPC = (route, args) => {
                if (route === '/tele_studio/edit_action') {
                    return true;
            ...
            await testUtils.dom.click(webClient.el.querySelector('.o_tele_studio_menu_item a'));

            // make list view disable and form view only will be there in studio view
            await testUtils.dom.click($(webClient.el).find('div[data-type="list"] .o_tele_studio_more'));
            ...
            await testUtils.dom.click($(webClient.el).find('div[data-type="form"] .o_tele_studio_more'));

...
import { systrayItem } from "@tele_studio/systray_item/systray_item";
import { makeFakeEnterpriseService } from "@tele_enterprise/../tests/mocks";
import { homeMenuService } from "@tele_enterprise/webclient/home_menu/home_menu_service";
import { studioService } from "@tele_studio/studio_service";
...
export async function openStudio(webClient, params = {}) {
    await click(webClient.el.querySelector(".o_main_navbar .o_tele_studio_navbar_item a"));
    await legacyExtraNextTick();
    if (params.noEdit) {
        const studioTabViews = webClient.el.querySelector(".o_tele_studio_menu_item a");
        await click(studioTabViews);
        const controlElm = webClient.el.querySelector(
            ".o_action_manager .o_tele_studio_editor .o_tele_studio_views"
        );
        if (!controlElm) {
    ...
    if (params.report) {
        const studioTabReport = webClient.el.querySelectorAll(".o_tele_studio_menu_item a")[1];
        await click(studioTabReport);
        await legacyExtraNextTick();
        let controlElm = webClient.el.querySelector(
            ".o_action_manager .o_tele_studio_editor .o_tele_studio_report_kanban"
        );
        if (!controlElm) {
        ...
        controlElm = webClient.el.querySelector(
            ".o_action_manager .o_tele_studio_editor .o_tele_studio_report_editor_manager"
        );
        if (!controlElm) {
...
export async function leaveStudio(webClient) {
    await click(webClient.el.querySelector(".o_studio_navbar .o_tele_studio_leave a"));
...
export function getReportServerData() {
    ...
    const views = {
        "ir.actions.report,false,kanban": `
            <kanban class="o_tele_studio_report_kanban" js_class="studio_report_kanban">

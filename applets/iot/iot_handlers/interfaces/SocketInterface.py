...
from tele.applets.hw_drivers.main import iot_devices

_logger = logging.getLogger(__name__)

# Because drivers don't get loaded as normal Python modules but directly in
# load_iot_handlers called by Manager.run, the log levels that get applied to the tele
...
# be ERROR at the tele-bin level. In order to work around this, it's possible to
...
class SocketInterface(Interface):
    ...
    def replace_socket_device(self, dev, addr):
        ...
        # problems since the key is also reported to the Tele database, which means a

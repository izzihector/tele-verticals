...
from tele.applets.phone_validation.tools import phone_validation
from tele.applets.test_mail_full.tests.common import TestMailFullCommon
...
class TestMassSMSInternals(TestMassSMSCommon):
    ...
    @mute_logger('tele.applets.mail.models.mail_render_mixin')
...
class TestMassSMS(TestMassSMSCommon):
    ...
    def test_mass_sms_links(self):
        mailing = self.env['mailing.mailing'].browse(self.mailing_sms.ids)
        mailing.write({
            'body_plaintext': 'Dear {{ object.display_name }} this is a mass SMS with two links http://www.tele.studio/smstest and http://www.tele.studio/smstest/{{ object.name }}',
        ...
        self.assertSMSTraces(
            ...
            mailing, self.records,
            sms_links_info=[[
                ('http://www.tele.studio/smstest', True, {}),
                ('http://www.tele.studio/smstest/%s' % record.name, True, {}),
    ...
    def test_mass_sms_partner_only(self):
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.tele.studio/smstest', True, {}),
                ('http://www.tele.studio/smstest/%s' % record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ] for record in records],
        )
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.tele.studio/smstest', True, {}),
                ('http://www.tele.studio/smstest/%s' % new_record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ]],
        )

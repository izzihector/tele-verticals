tele.define('tele_enterprise.list_tests', function (require) {
"use strict";
...
QUnit.module('tele_enterprise', {
    ...
    QUnit.test("add custom field button with other optional columns - studio not installed", async function (assert) {
        ...
        let listPatch;
        try {
            listPatch = unpatch(ListRenderer.prototype, 'tele_studio.ListRenderer');
        } catch (e) {
        ...
        if (listPatch) {
            patch(ListRenderer.prototype, 'tele_studio.ListRenderer', listPatch);
        }
        list.destroy();
    });

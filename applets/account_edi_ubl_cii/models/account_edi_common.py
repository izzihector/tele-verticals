...
from tele import _, models
from tele.tools import float_repr
from tele.tests.common import Form
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line_form, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the tele values
        ...
        It is not possible to do it in Tele.
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    tele_xmlid for tele_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]

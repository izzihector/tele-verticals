...
class AccountDisallowedExpensesReport(models.AbstractModel):
    ...
    def _get_options(self, previous_options=None):
        ...
        rg = self.env['automobile.disallowed.expenses.rate'].read_group(period_domain, ['rate'], 'vehicle_id')
    ...
    def _get_query(self, options, line_id):
        ...
        select += """,
                COALESCE(MAX(automobile_rate.rate), 0) as automobile_rate,
                vehicle.id as vehicle_id,
                vehicle.name as vehicle_name"""
        from_ += """
                LEFT JOIN automobile_vehicle vehicle ON aml.vehicle_id = vehicle.id
                LEFT JOIN automobile_disallowed_expenses_rate automobile_rate ON automobile_rate.id = (
                    SELECT r2.id FROm automobile_disallowed_expenses_rate r2
                    JOIN automobile_vehicle v2 ON r2.vehicle_id = v2.id
        ...
        group_by += options['multi_rate_in_period'] and ", automobile_rate.rate" or ""
        order_by = """
            ORDER BY category_code, vehicle.name IS NOT NULL, vehicle.name, account_code"""
        order_by_rate += ", automobile_rate"
    ...
    def _get_applicable_rate(self, values):
        ...
        return values['automobile_rate'] or super()._get_applicable_rate(values)

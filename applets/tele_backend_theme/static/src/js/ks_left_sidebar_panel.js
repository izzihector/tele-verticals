tele.define(
  "tele_backend_theme.ks_left_sidebar_panel",
  function (require) {
    "use strict";
...
//    var { NavBar } = require("@web/webclient/navbar/navbar");
    var { EnterpriseNavBar } =  require("@tele_enterprise/webclient/navbar/navbar");
...
//    var { MenusService } = require("@web/webclient/menus/menu_service");
    ...
    var ksBookmarks = require("tele_backend_theme.ks_bookmarks");
      ...
      willPatch() {
        ...
        if (this.ks_menu_bar == "Horizontal") {
          ...
          if ($('.ks_mobile_nav_bottom').length ===0) {
            $("div.tele-phone-profile").after('<div class="ks_mobile_nav_bottom">'+logout_url+'</div>');
          }
//          $("div.ks_mobile_nav_bottom").append(ks_my_profile);
//          $("div.ks_mobile_nav_bottom").append(ks_logout);
//          $("nav.o_main_navbar .tele-phone-side-menu").on("click",".tele-phone-menu-list .ks_mobile_nav_bottom a[data-menu]",
//            function (ev) {
//              ev.preventDefault();
...
//          $("div.ks_left_sidebar_panel .ks_app_sidebar .inner-sidebar").on("click",".tele-phone-menu-list .tele-phone-profile a[data-menu]",
//            function (ev) {
//              ev.preventDefault();
//              var menu = $(this).data("menu");
//              self.ksMobileUserMenu[
...
//          $("div.ks_left_sidebar_panel .ks_app_sidebar .inner-sidebar").on("click",".tele-phone-menu-list .ks_mobile_nav_bottom a[data-menu]",
//            function (ev) {
//              ev.preventDefault();
//              var menu = $(this).data("menu");
//              self.ksMobileUserMenu[
...
//          }
      ...
      // Apps Menu
        ...
        let ksTabs = document.querySelectorAll(".tele-tabs li");
        ksTabs.forEach((el, i) => {
...
//      },
      ...
      _ksOpenMobileDrawer: function () {
        // Append user's info
            ...
            if (ks_user_name) {
              $("div.tele-phone-profile .ks_user_name").text(ks_user_name);
            }
            if (ks_user_img_src) {
              $("div.tele-phone-profile .tele-user-profile-img img").attr(
                "src",
                ks_user_img_src
              );
            }

            if ($("div.tele-phone-side-menu").length){
              $("div.tele-phone-side-menu").addClass("active-menu");
              if((this.ks_menu_bar=='Vertical') &&($("ul.ks_menusections").length))
                $("div.tele-phone-profile").after($("ul.ks_menusections")[0].outerHTML);
              setTimeout(() => {
                $("div.tele-phone-menu-list").addClass("menu-show");
              }, 200);
            }
      },

      _ksCloseMobileDrawer: function () {
        if ($("div.tele-phone-menu-list").hasClass("menu-show")) {
          $("div.tele-phone-menu-list").removeClass("menu-show");
        if(this.ks_menu_bar=='Vertical')
          $("div.tele-phone-menu-list").find("ul.o_menu_sections").remove();
          setTimeout(() => {
            $("div.tele-phone-side-menu").removeClass("active-menu");
          }, 200);
        }
      },
      ...
      _onPageScroll: function (event) {
        ...
        if (event.deltaY > 0 && event.deltaY < 110 && $('#AllApps').hasClass('active')){
           if ($('.tele-tabs li.active').next().length > 0){
              var li = $('.tele-tabs li.active').next();
              $('.tele-tabs li.active').removeClass("active");
              li.addClass("active");
        ...
        // scroll up
           ...
           if ($('.tele-tabs li.active').prev().length > 0){
              var li = $('.tele-tabs li.active').prev();
              $('.tele-tabs li.active').removeClass("active");
              li.addClass("active");
              var ui = $("#AllApps > div.active");
      ...
      _ksManagerDrawer(drawer_status) {
        $('#ks_input_text').focus();
        this.$search_container.removeClass("tele-find-values");

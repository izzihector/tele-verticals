tele.define("tele_backend_theme.AbstractWebClient", function (require) {
  "use strict";
  ...
  patch(WebClient.prototype, "AbstractWebClient", {
    async setup() {
      ...
      const ksTitle = await this.env.services.rpc(
        ...
        const manifest =  await this.env.services.rpc("/tele_backend_theme/get_manifest").then(
        function (result) {
          if (result){
...
//      await ajax
//        .jsonRpc({ route: "/tele_backend_theme/get_manifest" }).then(
//        function (result) {
//          if (result){
//              let encodeData = encodeURIComponent(JSON.stringify(result));
//              let manifestURL = "data:application/manifest+json," + encodeData;

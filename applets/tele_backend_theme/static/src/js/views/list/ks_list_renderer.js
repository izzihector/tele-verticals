tele.define("tele_backend_theme.ks_list_renderer", function (require) {
  "use strict";

  var ajax = require("web.ajax");
  var ListRenderer = require("web.ListRenderer");
  var KsListDocumentViewer = require("tele_backend_theme.KsListDocumentViewer");
  ...
  var Ks_ListRenderer = ListRenderer.include({
    ...
    _onksViewAttachment: function (ev) {
      ...
      var rec_id = parseInt($(ev.currentTarget).data("tele-rec-id"));
          ...
          att_data[record.data.id].every((attachment, index, arr) => {
            if (index < ks_attachment_limit) {
              var $att_div = $("<div>", {
                ...
                "data-tele-rec-id": record.data.id,
              });
    ...
    _create_separator_for_split_window: function(fragment, split_mode){
        ...
        self.split_view_data.$split_separator.on("dragstop", function(event, ui) {
          ...
          self._rpc({
            route: "/tele_backend_theme/save/ks_splitted_form_width",
            params: {
              'data': val
            }
          });

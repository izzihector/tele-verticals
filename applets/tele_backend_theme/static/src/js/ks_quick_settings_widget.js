tele.define("tele_backend_theme.ks_quick_settings_widget", function(require) {
    "use strict";
    ...
    var KsQuickSettings = Widget.extend({
        ...
        events: {
            ...
            "click .tele-dropdown-close": "_ksHideUserSettings",
       ...
       **/
        willStart: async function() {
            ...
            const ks_current_theme = await this._rpc({
                route: "/tele_backend_theme/getTheme",
                params: {},
            });
            ...
            session.tele_backend_theme_data = data;
            ...
            if (ks_dynamic_css && ks_dynamic_css.ks_button_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/buttons/${ks_dynamic_css.ks_button_style}.css`);
            }
            if (ks_dynamic_css && ks_dynamic_css.ks_theme_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/theme_style/${ks_dynamic_css.ks_theme_style}.css`);
            }
            if (ks_dynamic_css && ks_dynamic_css.ks_tab_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/tab_style/${ks_dynamic_css.ks_tab_style}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_font_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/fonts/${ks_dynamic_css.ks_font_style}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_separator_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/separators/${ks_dynamic_css.ks_separator_style}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_checkbox_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/checkbox/${ks_dynamic_css.ks_checkbox_style}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_font_size) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/fontsize/${ks_dynamic_css.ks_font_size}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_chatter == "ks_chatter_right") {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/chatter/${ks_dynamic_css.ks_chatter}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_radio_button_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/radiobutton/${ks_dynamic_css.ks_radio_button_style}.css`);
            }

            if (ks_dynamic_css && ks_dynamic_css.ks_popup_animation_style) {
                ajax.loadCSS(`/tele_backend_theme/static/src/scss/components/popup_animation/${ks_dynamic_css.ks_popup_animation_style}.css`);
            }
...
//            this._ksManageCurrentColorTheme();
        ...
        _ksManageCurrentColorTheme() {
            ...
            if (session.ks_current_color_mode == "tele-light" && ks_header_icon_clr == "white") {
            ...
            if (session.ks_current_color_mode == "tele-light" && !ks_current_theme.default_theme) {
                document.body.style.setProperty("--tele-main-control-bg", "#FFFFFF");
            ...
            if (ks_current_theme.primary) {
                ...
                if (ks_current_theme["body-background"] == ks_current_theme.primary && session.ks_current_color_mode == "tele-light") {
            ...
            if (session.ks_current_color_mode == "tele-light") {
                ...
                if (!this.data.ks_body_background_image_enable) {
                    ...
                    if (ks_current_theme["tele-over-link"]) {
                        document.body.style.setProperty("--tele-over-link", ks_current_theme["tele-over-link"]);
                    }
        ...
        _onInputChange: function(ev) {
            ...
            if (ev.target.name) {
                ...
                if ($(ev.currentTarget).hasClass("ks_binary_field")) {
                    ...
                    if (file_node.getAttribute("data-model")) {
                        ...
                        utils.getDataURLFromFile(file).then(function(data) {
                            ...
                            ajax.jsonRpc("/tele_backend_theme/add/images", "call", {
                                image_info: {
                                    key: field_name,
                                    value: data
                                },
                                ...
                                // Reattach tele-zoom functionality.
                ...
                if (_field == "ks_auto_dark_mode" && _value && !session.tele_backend_theme_data.ks_sun_time_info) {
...
//                    this.do_warn(_t("Error"), _t("Please add sunrise and sunset time from user's preferences."));
        ...
        _ksApplyTempColorTheme: function(field_name, field_val) {
            var ks_color_field_dict = {
                ...
                ks_menu_hover_user: "--tele-over-link",
        ...
        _ksGlobalSettings: function(ev) {
            ...
            var dict = {
                ...
                context: {
                    form_view_ref: "tele_backend_theme.ks_global_configuration_form",
                },

            };
            if($(document).find(".o_home_menu_background").length){
                $(".o_web_client").removeClass("o_home_menu_background");
            }
        ...
        _ksResetValues: function() {
            ...
            for (var index in this.ks_unsaved_setting) {
                if (typeof session.tele_backend_theme_data[index] == "boolean") {
                    if (index in self.ks_dirty_data) {
                        self.ks_dirty_data[index].prop("checked", session.tele_backend_theme_data[index]);
                    } else {
                        $("input#" + index).prop("checked", session.tele_backend_theme_data[index]);
                    }
                    if (index == "ks_dark_mode") {
                        if (session.tele_backend_theme_data[index])
                    ...
                    delete this.ks_unsaved_setting[index];
                        ...
                        $("input#" + session.tele_backend_theme_data[index]).prop("checked", true);
                    delete this.ks_unsaved_setting[index];
            ...
            if ("ks_dark_mode"in this.ks_unsaved_setting) {
                this._ksManageAutoDarkMode(session.tele_backend_theme_data.ks_dark_mode);
            }
            // Hide/Show Auto-dark mode
            if ("ks_dark_mode"in this.ks_unsaved_setting) {
                this._ksManageAutoDarkMode(session.tele_backend_theme_data.ks_dark_mode);
        ...
        _KsResetDirtyData: function(session) {
            ...
            for (var index in self.ks_dirty_data) {
                if (index && self.ks_dirty_data[index] && (index in self.ks_dirty_data) && self.ks_dirty_data[index].attr('type') == 'range') {
                    ...
                    self.ks_dirty_data[index].val(session.tele_backend_theme_data[index])
                    ...
                    var active_rec = session.tele_backend_theme_data[self.ks_dirty_data[index].data('field-save')].filter(x=>x.ks_active)
                    ...
                    var val = session.tele_backend_theme_data[self.ks_dirty_data[index].attr('name')]
                    ...
                    $('input[name=' + self.ks_dirty_data[index].attr('name') + '][data-value="' + session.tele_backend_theme_data[self.ks_dirty_data[index].data('field-save')] + '"]').prop('checked', true);
                }
            }
            self.ks_dirty_data = {}
        },

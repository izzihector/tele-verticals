tele.define("tele_backend_theme.ks_global_config_widget", function(require) {
    "use strict";
       ...
       **/
        ...
        _onInputChange: function(ev) {
            ...
            if (!$(ev.currentTarget).parents('#global_theme_edit_section').length) {
                ...
                if(ev.currentTarget.value<=0.3){
                    document.getElementById("tele-drawer-global-slider").classList.remove("d-none");
                  }
                  else{
                    if(document.getElementById("tele-drawer-global-slider")){
                        document.getElementById("tele-drawer-global-slider").classList.add("d-none");
                    }
                  }
                // Manage data for text fields.
                if (ev.currentTarget.name && ev.currentTarget.dataset.type == "tele-char") {
                    this.ks_unsaved_theme_global_data[ev.currentTarget.name] = ev.currentTarget.value;
                    self.ks_dirty_data[ev.currentTarget.name] = $(ev.currentTarget);
                }
                    ...
                    if (file_node.getAttribute("data-model")) {
                        ...
                        utils.getDataURLFromFile(file).then(function(data) {
                            ...
                            if($(ev.currentTarget).hasClass("ks_login_background_image")){
                                ajax.jsonRpc("/tele_backend_theme/add/login/images", "call", {
                                    image_info: {
                                        key: field_name,
                                        value: data
                                    }
        ...
        _ksApplyTempColorTheme: function(field_name, field_val) {
            var ks_color_field_dict = {
                ...
                ks_menu_hover_global: "--tele-over-link",
        ...
        _ksLoginPageBgColor: function(){
            ...
            ajax.jsonRpc("/tele_backend_theme/add/login/color", "call", {
                data: {

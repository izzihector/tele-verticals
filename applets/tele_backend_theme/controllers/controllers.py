# -*- coding: utf-8 -*-
from tele.http import request, route, _logger
import requests
...
class KsCurvedBackendTheme(http.Controller):
    ...
    @route(['/tele_backend_theme/save/form_width'], type='json', auth='user')
    def ks_save_form_page_width(self, val):
        request.env.user.write({
            'ks_form_page_width': val
        })

    @route(['/tele_backend_theme/save/ks_splitted_form_width'], type='json', auth='user')
    ...
    def update_bookmarks(self, **post):
        ...
        return request.env['ir.ui.view']._render_template("tele_backend_theme.ks_bookmark_", values)
    ...
    def render_theme_view_settings(self, ks_setting_scope, ks_rec_id=None):
        ...
        if ks_setting_scope == 'User':
            ...
            obj_list = {
                'global': request.env.ref('tele_backend_theme.ks_global_config_single_rec'),
            ...
            values.update({
                ...
                'ks_manager_role': user.has_group('tele_backend_theme.ks_curved_theme_settings'),
                'ks_global_config_id': request.env.ref('tele_backend_theme.ks_global_config_single_rec').id,
            ...
            if ks_current_menu_bar_scope == 'Global':
                ks_origin_data = request.env.ref('tele_backend_theme.ks_global_config_single_rec')
    ...
    @route(['/tele_backend_theme/save/split_data'], type='json', auth='user')
    ...
    @route(['/tele_backend_theme/add/images'], type='json', auth='user')
    ...
    @route(['/tele_backend_theme/add/login/images'], type='json', auth='user')
    ...
    @route(['/tele_backend_theme/add/login/color'], type='json', auth='user')
    ...
    def save_theme_settings(self, **post):
        ...
        if post.get('ks_origin_scope') == 'global':
            ks_origin_obj = request.env.ref('tele_backend_theme.ks_global_config_single_rec')
            ks_splitter = '_global'
        ...
        if 'ks_enterprise_apps' in post['ks_unsaved_setting']:
            request.env.ref('tele_backend_theme.ks_curved_theme_hide_enterprise_apps').write({
    ...
    def ks_get_website_title(self, **post):
        ks_origin = request.env.ref('tele_backend_theme.ks_global_config_single_rec')
    ...
    def set_background_default(self, ks_setting_scope, ks_rec_id=None, ks_default_info=None):
        if ks_setting_scope == 'Global' and ks_default_info:
            ks_global_obj = request.env.ref('tele_backend_theme.ks_global_config_single_rec')
    ...
    @http.route('/tele_backend_theme/getTheme', type='json', auth="user")
    def ks_get_theme(self):
        ...
        if ks_current_theme_data:
            return {
                ...
                'tele-over-link': ks_current_theme_data.ks_menu_hover,
        ...
        return {
            ...
            'tele-over-link': '#f5f5f5',
            'tab-bg': '#ffffff',
            'primary-btn': '#0077D2',
            'heading-color': '#0077D2',
            'link-color': '#0077D2',
...
class KsHome(Home):
    ...
    def ks_get_login_page_logo(self):
        ks_global_obj = request.env.ref('tele_backend_theme.ks_global_config_single_rec').sudo()

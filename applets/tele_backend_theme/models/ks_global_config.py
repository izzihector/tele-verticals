...
from tele import models, fields, api, _, exceptions
...
class KsThemeGlobalConfig(models.Model):
    ...
    ks_website_title = fields.Char(string="Website Backend Title", default='Tele')
    ks_click_edit = fields.Boolean(string="Double-click Edit", default=False)
    ...
    scope_ks_enterprise_apps = fields.Selection(string="Scope of Enterprise Apps",
                                                selection=[('User', 'User'), ('Company', 'Company'),
                                                           ('Global', 'Global')],
                                                default='User')
    scope_ks_tele_referral = fields.Selection(string="Scope of Tele referral",
                                              selection=[('User', 'User'), ('Company', 'Company'),
                                                         ('Global', 'Global')],
                                              default='User')
    ...
    def ks_get_config_values(self):
        ...
        :return:
        """
        ks_global_config_id = self.env.ref('tele_backend_theme.ks_global_config_single_rec').id
    ...
    def get_body_background(self, ks_fields):
        ...
        :return:
        """
        ks_global_config_id = self.env.ref('tele_backend_theme.ks_global_config_single_rec')
        ks_image_model = {
    ...
    def ks_get_value_from_scope(self, ks_field_list):
        ks_global_origin = self.env.ref('tele_backend_theme.ks_global_config_single_rec')
    ...
    def write(self, vals):
        if ('ks_pwa_icon' in vals) and not vals.get('ks_pwa_icon'):
            ks_prev_icons = self.env["ir.attachment"].sudo().search(
                [('url', 'like', '/tele_backend_theme/logo')])
            if ks_prev_icons:
    ...
    def ks_create_icons_size(self, ks_icon, size, ks_pwa_mimetype, extension):
        url = '/tele_backend_theme/logo/icon_' + str(size[0]) + '_' + request.db + extension

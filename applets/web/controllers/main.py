...
from tele.api import call_kw
from tele.applets.base.models.ir_qweb import render as qweb_render
from tele.modules import get_resource_path, module
from tele.tools import html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property, osutil
from tele.tools.mimetypes import guess_mimetype
from tele.tools.translate import _
from tele.tools.misc import str2bool, xlsxwriter, file_open, file_path
from tele.tools.safe_eval import safe_eval, time
from tele import http
from tele.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception
from tele.exceptions import AccessError, UserError, AccessDenied
from tele.models import check_method_name
from tele.service import db, security
...
def fix_view_modes(action):
    """ For historical reasons, Tele has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "tele-web" in x.auto_comments:
...
class HomeStaticTemplateHelpers(object):
    ...
    def __init__(self, applets, db, checksum_only=False, debug=False):
        '''
        :param str|list applets: plain list or comma separated list of applets
        :param str db: the current db we are working on
        :param bool checksum_only: only computes the checksum of all files for applets
        :param str debug: the debug mode of the session
        '''
        super(HomeStaticTemplateHelpers, self).__init__()
        self.applets = applets.split(',') if isinstance(applets, str) else applets
        self.db = db
        self.debug = debug
    ...
    def _get_asset_paths(self, bundle):
        ...
        """
        return request.env['ir.asset']._get_asset_paths(applets=self.applets, bundle=bundle, xml=True)
    ...
    @classmethod
    def get_qweb_templates_checksum(cls, applets=None, db=None, debug=False, bundle=None):
        return cls(applets, db, checksum_only=True, debug=debug)._get_qweb_templates(bundle)[1]

    @classmethod
    def get_qweb_templates(cls, applets=None, db=None, debug=False, bundle=None):
        return cls(applets, db, debug=debug)._get_qweb_templates(bundle)[0]
...
class Home(http.Controller):
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = tele.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (tele.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return tele.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = tele.tools.config.verify_admin_password('admin')
        d['list_db'] = tele.tools.config['list_db']
        d['langs'] = tele.service.db.exp_list_lang()
        d['countries'] = tele.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = tele.service.db.list_db_incompatible(d['databases'])
        except tele.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = tele.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            tele.service.db.check_super(master_pwd)
            ...
            dump_stream = tele.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def account(self):
        ...
        return 'https://accounts.tele.studio/oauth2/auth?' + url_encode(params)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = tele.modules.registry.Registry(dbname)
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: tele.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Tele's export data to whatever the

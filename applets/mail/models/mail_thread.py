...
from tele import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID, Command
from tele.exceptions import MissingError
from tele.osv import expression

from tele.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _get_mail_message_access(self, res_ids, operation, model_name=None):
        """ mail.message check permission rules for related document. This method is
            meant to be inherited in order to implement applets-specific behavior.
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not telebot
    ...
    def message_post(self, *,
        ...
        if values['author_id'] and values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(values['author_id']).active:  # we dont want to add telebot/inactive as a follower
    ...
    def _notify_record_by_email(self, message, recipients_data, msg_vals=False,
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in Tele)
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        """ Optional method to override in applets inheriting from mail.thread.
        ...
        with track_visibility set. Since Tele v7 it is considered as being

tele.define('web_editor.snippets.options', function (require) {
'use strict';
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.teleEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.teleEditor.automaticStepActive('preview_option');
        }
...
registry.sizing = SnippetOptionWidget.extend({
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ev.preventDefault();
            self.options.wysiwyg.teleEditor.automaticStepUnactive('resizing');
            ...
            var bodyMouseUp = function () {
                ...
                self.options.wysiwyg.teleEditor.automaticStepActive('resizing');
            };
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async crop() {
        ...
        new weWidgets.ImageCropWidget(this, this.$target[0]).appendTo(this.options.wysiwyg.teleEditor.document.body);
    ...
    async resetCrop() {
        ...
        await cropper.appendTo(this.options.wysiwyg.teleEditor.document.body);
        await cropper.reset();
        await this._reapplyCurrentShape();
    },
    /**
...
￼    */
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Tele).
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.teleEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.teleEditor.observerActive('_markColorLevel');
    },
});

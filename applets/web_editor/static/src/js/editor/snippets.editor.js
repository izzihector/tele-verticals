tele.define('web_editor.snippet.editor', function (require) {
'use strict';
...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function (shouldRecordUndo = true) {
        this.options.wysiwyg.teleEditor.unbreakableStepUnactive();
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.teleEditor.historyStep();
        }
    },
    /**
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.teleEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _onDragAndDropStart: function () {
        this.options.wysiwyg.teleEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.teleEditor.automaticStepUnactive();
        var self = this;
        this.dropped = false;
        this._dropSiblings = {
    ...
    _onDragAndDropStop: function (ev, ui) {
        this.options.wysiwyg.teleEditor.automaticStepActive();
        this.options.wysiwyg.teleEditor.automaticStepSkipStack();
        this.options.wysiwyg.teleEditor.unbreakableStepUnactive();
        ...
        this.options.wysiwyg.teleEditor.observerActive('dragAndDropMoveSnippet');
        if (this.dropped) {
            if (prev) {
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.teleEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.teleEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.teleEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.teleEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.teleEditor.historyCanRedo());
                };
                this.options.wysiwyg.teleEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.teleEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.teleEditor.editable).trigger('content_changed');
                });
                this.options.wysiwyg.teleEditor.addEventListener('historyRevert', _.debounce(() => {
        ...
        const smoothScrollOptions = this._getScrollOptions({
            jQueryDraggableOptions: {
                ...
                start: function () {
                    ...
                    const doc = self.options.wysiwyg.teleEditor.document;
                    $(doc.body).addClass('oe_dropzone_active');

                    self.options.wysiwyg.teleEditor.automaticStepUnactive();

                    self.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                    self.options.wysiwyg.teleEditor.observerUnactive('dragAndDropCreateSnippet');
                ...
                stop: async function (ev, ui) {
                    ...
                    self.options.wysiwyg.teleEditor.automaticStepActive();
                    self.options.wysiwyg.teleEditor.automaticStepSkipStack();
                    $toInsert.removeClass('oe_snippet_body');
                    ...
                    self.options.wysiwyg.teleEditor.observerActive('dragAndDropCreateSnippet');
                    ...
                    if (dropped) {
                        ...
                        _.defer(async function () {
                            ...
                            self.options.wysiwyg.teleEditor.unbreakableStepUnactive();
                            self.options.wysiwyg.teleEditor.historyStep();
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).teleBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.teleEditor.bindExecCommand($customizeTableBlock[0]);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.teleEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.teleEditor.editable));
    },
    /**

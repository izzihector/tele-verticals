tele.define('web_editor.wysiwyg', function (require) {
'use strict';
...
const Wysiwyg = Widget.extend({
    ...
    start: async function () {
        ...
        if (this.teleEditor.document !== document) {
            this.teleEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.teleEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.teleEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        this.teleEditor.addEventListener('contentChanged', function () {
        ...
        this.teleEditor.document.addEventListener('selectionchange', this._onSelectionChange);

        this.teleEditor.addEventListener('preObserverActive', () => {
            ...
            this._setONotEditable(this.teleEditor.editable);
        });

        return _super.apply(this, arguments).then(() => {
            if (this.options.autohideToolbar) {
                if (this.teleEditor.isMobile) {
                    $(this.teleEditor.editable).before(this.toolbar.$el);
                } else {
    ...
    setupCollaboration(collaborationChannel) {
        ...
        this._getNewPtp = () => {
            ...
            return new PeerToPeer({
                ...
                onRequest: {
                    ...
                    get_missing_steps: (params) => this.teleEditor.historyGetMissingSteps(params.requestPayload),
                    get_history_from_snapshot: () => this.teleEditor.historyGetSnapshotSteps(),
                    get_collaborative_selection: () => this.teleEditor.getCurrentCollaborativeSelection(),
                },
                onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                    switch (notificationName) {
                        case 'ptp_remove':
                            this.teleEditor.multiselectionRemove(notificationPayload);
                            break;
                        ...
                        case 'ptp_disconnect':
                            ...
                            this.teleEditor.multiselectionRemove(fromClientId);
                            break;
                        case 'rtc_data_channel_open': {
                            ...
                            if (!historySyncAtLeastOnce) {
                                ...
                                if (remoteSelection) {
                                    this.teleEditor.onExternalMultiselectionUpdate(remoteSelection);
                                }
                            }
                            break;
                        }
                        case 'oe_history_step':
                            ...
                            if (historySyncFinished) {
                                this.teleEditor.onExternalHistorySteps([notificationPayload]);
                            }
                            break;
                        case 'oe_history_set_selection': {
                            ...
                            this.teleEditor.onExternalMultiselectionUpdate(selection);
                            break;
                        }
                    }
                }
        ...
        const editorCollaborationOptions = {
            ...
            onHistoryMissingParentSteps: async ({ step, fromStepId }) => {
                ...
                this.ptp && this.teleEditor.onExternalHistorySteps(missingSteps.concat([step]));
            },
            onHistoryNeedSync: async () => {
                ...
                if (firstClientId !== this._currentClientId) {
                    ...
                    this.teleEditor.historyResetFromSteps(historySteps);
    ...
    destroy: function () {
        ...
        if (this.teleEditor) {
            this.teleEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.teleEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.teleEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.teleEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            for (const observer of this._oNotEditableObservers.values()) {
                observer.disconnect();
            }
            this.teleEditor.destroy();
        }
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.teleEditor.multiselectionRefresh, { passive: true });
        }
    ...
    renderElement: function () {
        ...
        if (this.options.resizable && !device.isMobile) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper tele-editor">');
            this.$root = $wrapper;
    ...
    getValue: function (options) {
        ...
        this.teleEditor.cleanForSave($editable[0]);
    ...
    cancel: function (reload) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.teleEditor.historySize().length) {
                resolve();
            } else {
    ...
    setValue: function (value) {
        ...
        this.teleEditor.sanitize();
        this.teleEditor.historyStep(true);
    },
    /**
     * Undo one step of change in the editor.
     */
    undo: function () {
        this.teleEditor.historyUndo();
    },
    /**
     * Redo one step of change in the editor.
     */
    redo: function () {
        this.teleEditor.historyRedo();
    },
    /**
     ...
     */
    focus: function () {
        if (this.teleEditor && !this.teleEditor.historyResetLatestComputedSelection()) {
            ...
            const selection = this.teleEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    },
    getDeepRange() {
        return getDeepRange(this.teleEditor.editable);
    },
    closestElement(...args) {
        return closestElement(...args);
    },
    isSelectionInEditable: function () {
        return this.teleEditor.isSelectionInEditable();
    },
    cleanForSave: async function () {
        this.teleEditor.clean();
    ...
    /**
     * Start or resume the Tele field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeTeleFieldChanges: function () {
        ...
        if (this.teleFieldObservers) {
            for (let observerData of this.teleFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const teleFieldSelector = '[data-oe-model], [data-oe-translation-id]';
            const $teleFields = this.$editable.find(teleFieldSelector);
            this.teleFieldObservers = [];

            $teleFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.teleEditor.filterMutationRecords(mutations);
                    if (!mutations.length) {
                        return;
                    }
                    let $node = $(field);
                    let $nodes = $teleFields.filter(function () {
                        return this !== field;
                    });
                ...
                this.teleFieldObservers.push({field: field, observer: observer});
            });
        }
    },
    /**
     ...
     */
    _pauseTeleFieldObservers: function () {
        for (let observerData of this.teleFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.teleEditor.document.getSelection().getRangeAt(0).startContainer;
    ...
    toggleLinkTools(options = {}) {
        const linkEl = getInSelection(this.teleEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.linkTools) {
                ...
                if (!this.linkTools || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkTools.$link[0])) {
                    const { link } = Link.getOrCreateLink({
                        containerNode: this.teleEditor.editable,
                    ...
                    this.linkTools = new weWidgets.LinkTools(this, {wysiwyg: this, noFocusUrl: options.noFocusUrl}, this.teleEditor.editable, linkToolsData, $btn, link );
                }
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.teleEditor.document.removeEventListener('click', _onClick, true);
                    }
                };
                this.teleEditor.document.addEventListener('click', _onClick, true);
            ...
            linkDialog.on('save', this, data => {
                ...
                this.teleEditor.setContenteditableLink(linkWidget.$link[0]);
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            ...
            this.teleEditor.execCommand('unlink');
        }
    },
    /**
    ...
    getInSelection(selector) {
        return getInSelection(this.teleEditor.document, selector);
    },
    ...
    _configureToolbar: function (options) {
        ...
        $toolbar.find('#image-crop').click(e => {
            ...
            new weWidgets.ImageCropWidget(this, this.lastMediaClicked).appendTo(this.teleEditor.document.body);
            this.teleEditor.toolbarHide();
            $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.teleEditor.toolbarShow());
        });
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.teleEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.teleEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.teleEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.teleEditor.document).on('mousedown', mousedown);
        });
        ...
        setTimeout(() => {
            ...
            if (!options.snippets && scrollableContainer.length) {
                this.teleEditor.addDomListener(
                    scrollableContainer[0],
                    'scroll',
                    this.teleEditor.updateToolbarPosition.bind(this.teleEditor),
                );
            }
        }, 0);
    },
        ...
        for (const elem of $dropdownContent) {
            ...
            $dropdown.on('show.bs.dropdown', () => {
                ...
                mutex.exec(() => {
                    ...
                    this.teleEditor.historyStash();
                    colorpicker = new ColorPaletteWidget(this, {
                        excluded: ['transparent_grayscale'],
                        $editable: $(this.teleEditor.editable), // Our parent is the root widget, we can't retrieve the editable section from it...
                    ...
                    colorpicker.on('custom_color_picked color_picked', null, ev => {
                        if (hadNonCollapsedSelection) {
                            this.teleEditor.historyResetLatestComputedSelection(true);
                        }
                        // Unstash the mutations now that the color is picked.
                        this.teleEditor.historyUnstash();
                    ...
                    colorpicker.on('color_hover', null, ev => {
                        ...
                        this.teleEditor.historyPauseSteps();
                        try {
                            this._processAndApplyColor(eventName, ev.data.color);
                        } finally {
                            this.teleEditor.historyUnpauseSteps();
                        }
                    });
                    colorpicker.on('color_leave', null, ev => {
                        this.teleEditor.historyRevertCurrentStep();
                    });
    ...
    _processAndApplyColor: function (eventName, color) {
        ...
        const fonts = this.teleEditor.execCommand('applyColor', color, eventName === 'foreColor' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        ...
        this.teleEditor.updateColorpickerLabels({
    ...
    _updateEditorUI: function (e) {
        ...
        if (isInMedia) {
            ...
            const selection = this.teleEditor.document.getSelection();
            const range = this.teleEditor.document.createRange();
        ...
        const link = getInSelection(this.teleEditor.document, this.customizableLinksSelector);
    ...
    _updateMediaJustifyButton: function (commandState) {
        ...
        // Change the ID to prevent TeleEditor from controlling it as this is
        ...
        const commands = [
            {
                ...
                callback: () => {
                    this.teleEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                groupName: _t('Basic blocks'),
                ...
                fontawesome: 'fa-code',
                callback: () => {
                    this.teleEditor.execCommand('setTag', 'pre');
                },
            },
            {
    ...
    _saveElement: function ($el, context, withLang) {
        ...
        // remove ZeroWidthSpace from tele field value
        // ZeroWidthSpace may be present from TeleEditor edition process
    ...
    _onSelectionChange() {
        if (this.options.autohideToolbar) {
            ...
            if (isVisible && !this.teleEditor.document.getSelection().isCollapsed) {
                this.linkPopover.hide();
            }
        }
    },
    ...
    _signalOnline: async function () {
        ...
        try {
            ...
            if (currentContent !== dbContent && dbRecordDate !== currentRecordDate) {
                ...
                $dialogContent.append($(this.teleEditor.editable).clone());
    ...
    _resetEditor: function (value) {
        ...
        this._startCollaborationTime = new Date().getTime();
        this.ptp = this._getNewPtp();
        this.teleEditor.collaborationSetClientId(this._currentClientId);
        this.setValue(value);
        this.teleEditor.historyReset();
                    ...
                    for (const record of records) {
                        if (record.type === 'attributes' && record.attributeName === 'class') {
                            ...
                            if (!node.classList.contains('o_not_editable')) {
                                this.teleEditor.observerUnactive('_setONotEditable');
                                node.removeAttribute('contenteditable');
                                this.teleEditor.observerActive('_setONotEditable');
                                observer.disconnect();
...
tele.define('web_editor.widget', function (require) {
'use strict';
    return {

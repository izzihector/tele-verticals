tele.define('wysiwyg.widgets.Link', function (require) {
'use strict';
...
const Link = Widget.extend({
    ...
    init: function (parent, options, editable, data, $button, link) {
        ...
        if (this.data.range) {
            this.$link = this.$link || $(TeleEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
        ...
        if (!this.data.url) {
            const urls = this.data.content.match(TeleEditorLib.URL_REGEX_WITH_INFOS);
            if (urls) {
    ...
    _onURLInput: function () {
        ...
        this.options.wysiwyg && this.options.wysiwyg.teleEditor.historyPauseSteps('_onURLInput');
        this._adaptPreview();
        this.options.wysiwyg && this.options.wysiwyg.teleEditor.historyUnpauseSteps('_onURLInput');
    },

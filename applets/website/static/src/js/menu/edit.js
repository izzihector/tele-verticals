tele.define('website.editMenu', function (require) {
'use strict';
...
var EditPageMenu = websiteNavbarData.WebsiteNavbarActionWidget.extend({
    ...
    async _createWysiwyg() {
        ...
        // Only make the tele structure and fields editable.
        this.wysiwyg.teleEditor.observerUnactive();
        $('#wrapwrap').on('click.tele-website-editor', '*', this, this._preventDefault);
        ...
        const processRecords = (records) => {
            ...
            this.wysiwyg.teleEditor.automaticStepSkipStack();
        ...
        this.wysiwyg.teleEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.wysiwyg.teleEditor.addEventListener('observerActive', observe);

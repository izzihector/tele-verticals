...
from tele import exceptions
from tele.applets.mail.tests.common import mail_new_test_user
from tele.applets.test_mail.data import test_mail_data
from tele.applets.test_mail.data.test_mail_data import MAIL_TEMPLATE
from tele.applets.test_mail.models.test_mail_models import MailTestGateway
from tele.applets.test_mail.tests.common import TestMailCommon
from tele.tests import tagged
from tele.tests.common import users
from tele.tools import email_split_and_format, formataddr, mute_logger
...
class TestEmailParsing(TestMailCommon):
    ...
    def test_message_parse_eml_bounce_headers(self):
        ...
        msg_id = '<861878175823148.1577183525.736005783081055-tele-19177-account.invoice@mycompany.example.com>'
        mail = self.format(
...
class TestMailgateway(TestMailCommon):
    ...
    def _create_gateway_message(cls, record, msg_id_prefix, **values):
        msg_values = {
            ...
            'message_id': f'<{msg_id_prefix}-tele-{record.id}-{record._name}@{socket.gethostname()}>',
            'message_type': 'email',
            'model': record._name,
            'res_id': record.id,
    ...
    @mute_logger('tele.applets.mail.models.mail_thread', 'tele.models')
    ...
    @mute_logger('tele.applets.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not telebot: added as follower """
        ...
        telebot.active = True
        telebot.email = 'telebot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, telebot.email_formatted, 'groups@test.com',
                subject='Telebot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, telebot)
        self.assertEqual(record4.message_ids[0].email_from, telebot.email_formatted)
    ...
    @mute_logger('tele.applets.mail.models.mail_thread', 'tele.models.unlink', 'tele.applets.mail.models.mail_mail')
    ...
    @mute_logger('tele.applets.mail.models.mail_thread', 'tele.applets.mail.models.mail_mail', 'tele.models.unlink')
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by tele should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        tele should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since Tele saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process, MAIL_TEMPLATE,
                          self.partner_1.email_formatted, 'noone@test.com', subject='spam',
                          extra='In-Reply-To: <12321321-tele-%d-mail.test.gateway@%s>' % (self.test_record.id, socket.gethostname()))

        # when 6.1 messages are present, compat mode is available
        # Tele 10 update: compat mode has been removed and should not work anymore

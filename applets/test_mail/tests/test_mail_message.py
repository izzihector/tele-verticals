...
from tele.applets.test_mail.models.test_mail_models import MailTestSimple
from tele.exceptions import AccessError, UserError
from tele.tools import is_html_empty, mute_logger, formataddr
from tele.tests import tagged, users
...
class TestMessageValues(TestMailCommon):
    ...
    @mute_logger('tele.models.unlink')
    ...
    def test_mail_message_values_fromto_document_alias(self):
        ...
        self.assertIn('-tele-%d-mail.test' % self.alias_record.id, msg.message_id.split('@')[0])
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        self.assertIn('-tele-%d-mail.test.simple' % test_record.id, msg.message_id.split('@')[0])
...
class TestMessageAccess(TestMailCommon):
    ...
    @mute_logger('tele.applets.base.models.ir_model')
    ...
    def test_mail_message_access_create_wo_parent_access(self):
        ...
        partner_1 = self.env['res.partner'].create({
            'name': 'Jitendra Prajapati (jpr-tele)',
            'email': 'jpr@tele.studio',
        })

...
from tele.applets.iap.tools import iap_tools
...
class HrExpense(models.Model):
    ...
    def action_submit_expenses(self, **kwargs):
        ...
        for expense in self.filtered(lambda x: x.extract_state == 'waiting_validation'):
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                'hr_expense_extract_endpoint', 'https://iap-extract.tele.studio') + '/api/extract/expense/1/validate_batch'
    ...
    def _check_status(self):
        self.ensure_one()
        endpoint = self.env['ir.config_parameter'].sudo().get_param(
            'hr_expense_extract_endpoint', 'https://iap-extract.tele.studio') + '/iap/expense_extract/get_result'
        params = {
    ...
    def retry_ocr(self):
        ...
        if attachments and attachments.exists() and self.extract_state in ['no_extract_requested', 'not_enough_credit', 'error_status', 'module_not_up_to_date']:
            ...
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                    'hr_expense_extract_endpoint', 'https://iap-extract.tele.studio') + '/iap/expense_extract/parse'
...
<p>Snap pictures of your receipts and let Tele<br/> automatically create expenses for you.</p>
<p class="d-none d-md-block">
    <a href="https://apps.apple.com/be/app/tele/id1272543640" target="_blank">
        <img alt="Apple App Store" class="img img-fluid h-100 o_expense_apple_store" src="/hr_expense/static/img/app_store.png"/>
    </a>
    <a href="https://play.google.com/store/apps/details?id=com.tele.mobile" target="_blank">

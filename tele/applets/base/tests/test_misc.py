...
from tele.tools import config, misc, date_utils, file_open, file_path, merge_sequences, remove_accents
from tele.tests.common import TransactionCase, BaseCase
...
class TestAppletsFileAccess(BaseCase):
    ...
    def test_file_path(self):
        ...
        # leading 'applets/' is ignored if present
        self.assertTrue(file_path("applets/web/__init__.py"))
        relpath = os.path.join('applets', relpath) # 'applets/base/tests/test_misc.py'
        self.assertEqual(__file__, file_path(relpath))

        # files in root_path are allowed
        self.assertTrue(file_path('tools/misc.py'))

        # errors when outside applets_paths
    ...
    def test_file_open(self):
        ...
        # leading 'applets/' is ignored if present
        self.assertCanRead("applets/web/__init__.py", "import")
        relpath = os.path.join('applets', relpath) # 'applets/base/tests/test_misc.py'
        self.assertCanRead(relpath, test_needle)

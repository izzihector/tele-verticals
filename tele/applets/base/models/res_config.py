...
from tele import api, models, _, Command
from tele.exceptions import AccessError, RedirectWarning, UserError
...
class ResConfigInstaller(models.TransientModel, ResConfigModuleInstallationMixin):
    """ New-style configuration base specialized for applets selection
    and installation.
    ...
    should be the names of the applets to install (when selected). Upon action
    execution, selected boolean fields (and those only) will be interpreted as
    applets to install, and batch-installed.
    ...
    property. This property is a mapping of a collection of applets (by
    name) to a collection of applets (by name) [#]_, and if all the *key*
    applets are selected for installation, then the *value* ones will
    ...
    context) and can return a collection of additional applets to
    ...
    returns a ``set`` of applets to install (applets selected by the
    above methods minus applets from the *basic* set which are already
    ...
    installation, even for already installed applets.
    ...
    def modules_to_install(self):
        ...
        * additionals, additionals are setup through the ``_install_if``
          ...
          applets in the value are added to the set of modules to install
    ...
    def execute(self):
        ...
        _logger.info('Selecting applets %s to install', to_install)
...
class ResConfigSettings(models.TransientModel, ResConfigModuleInstallationMixin):
    ...
    def get_config_warning(self, msg):
        ...
        Example of use:
        ---------------
        from tele.applets.base.models.res_config import get_warning_config

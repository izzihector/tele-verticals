...
import tele
from tele.tools import misc
from tele import tools
from tele import api, fields, http, models
...
class IrAsset(models.Model):
    ...
    def _get_asset_paths(self, bundle, applets=None, css=False, js=False, xml=False):
        """
        ...
        3. The manifests of the given applets are checked for assets declaration
        ...
        if applets is None:
            applets = self._get_active_applets_list()

        asset_paths = AssetPaths()
        self._fill_asset_paths(bundle, applets, installed, css, js, xml, asset_paths, [])
        return asset_paths.list

    def _fill_asset_paths(self, bundle, applets, installed, css, js, xml, asset_paths, seen):
        """
        Fills the given AssetPaths instance by applying the operations found in
        the matching bundle of the given applets manifests.
        ...
        if not root._loaded:
            root.load_applets()
            root._loaded = True
        manifest_cache = http.applets_manifest
        exts = []
        if js:
        ...
        def process_path(directive, target, path_def):
            ...
            list of applets, extensions, asset_paths and manifest_cache.
    ...
    def _get_active_applets_list(self):
        """Can be overridden to filter the returned list of active modules."""
        return self._get_installed_applets_list()

    @api.model
    @tools.ormcache('applets_tuple')
    def _topological_sort(self, applets_tuple):
        ...
        manifs = map(mapper, applets_tuple)
    ...
    def _get_installed_applets_list(self):
        """
        Returns the list of all installed applets.
        ...
        return self.env.registry._init_modules | set(tele.conf.server_wide_modules or []) | set(self.env.context.get('install_module', []))

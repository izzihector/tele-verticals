...
from tele.tools import ustr, pycompat, formataddr, email_normalize, encapsulate_email, email_domain_extract, email_domain_normalize


_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('tele.tests')
...
class IrMailServer(models.Model):
    ...
    from_filter = fields.Char(
      ...
      'e.g.: "notification@tele.studio" or "tele.studio"')
    ...
    def _get_test_email_addresses(self):
        ...
        if self.from_filter:
            if "@" in self.from_filter:
                ...
                return self.from_filter, "noreply@tele.studio"
            # All emails will be sent from any address in the same domain
            default_from = self.env["ir.config_parameter"].sudo().get_param("mail.default.from", "tele")
            return f"{default_from}@{self.from_filter}", "noreply@tele.studio"
        ...
        return email_from, 'noreply@tele.studio'
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the tele-make argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the tele-make argument "smtp_ssl_private_key"
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Tele Server does not support SMTP-over-SSL. "
        ...
        # Store the "from_filter" of the mail server / tele-make argument to  know if we
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-tele".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-tele')
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        - Mail server to use to send the email (None if we use the tele-make arguments)
        ...
        # 5: SMTP config in tele-make arguments
        from_filter = self.env['ir.config_parameter'].sudo().get_param(
            'mail.default.from_filter', tools.config.get('from_filter'))

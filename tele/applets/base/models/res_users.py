...
from tele import api, fields, models, tools, SUPERUSER_ID, _, Command
from tele.applets.base.models.ir_model import MODULE_UNINSTALL_FLAG
from tele.exceptions import AccessDenied, AccessError, UserError, ValidationError
from tele.http import request
from tele.modules.module import get_module_resource
from tele.osv import expression
from tele.service.db import check_super
from tele.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    """ User class. A res.users record models an Tele user and is different
        from an employee.
    ...
    def _unlink_except_superuser(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Tele (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: tele.exceptions.AccessDenied when old password is wrong
        :raise: tele.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
    ...
    def _is_admin(self):
        self.ensure_one()
        return self._is_superuser() or self.has_group('base.group_tele_manager')
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Tele is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Tele in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.tele.studio/documentation/15.0/administration/install/deploy.html#https for details.",
                    source
                )

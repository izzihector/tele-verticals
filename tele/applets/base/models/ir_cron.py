...
BASE_VERSION = tele.modules.load_information_from_description_file('base')['version']
...
class ir_cron(models.Model):
    ...
    def create(self, values):
        ...
        if os.getenv('TELE_NOTIFY_CRON_CHANGES'):
    ...
    def _process_jobs(cls, db_name):
        ...
        try:
            db = tele.sql_db.db_connect(db_name)
            ...
            with db.cursor() as cron_cr:
                ...
                for job_id in (job['id'] for job in jobs):
                    ...
                    registry = tele.registry(db_name)
            ...
            if e.pgcode == '42P01':
                ...
                # The table ir_cron does not exist; this is probably not an Tele database.
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        tele.modules.reset_modules_state(cr.dbname)
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            tele.netsvc.log(_logger, logging.DEBUG, 'cron.object.execute', (self._cr.dbname, self._uid, '*', cron_name, server_action_id), depth=log_depth)
            start_time = False
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('TELE_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('TELE_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)

    def _notifydb(self):
        """ Wake up the cron workers
        The TELE_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with tele.sql_db.db_connect('postgres').cursor() as cr:

# -*- coding: utf-8 -*-
"""
The module :mod:`tele.tests.common` provides unittest test cases and a few
...
from tele.models import BaseModel
from tele.exceptions import AccessError
from tele.modules.registry import Registry
from tele.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from tele.service import security
from tele.sql_db import BaseCursor, Cursor
from tele.tools import float_compare, single_email_re, profiler
from tele.tools.misc import find_in_path
...
# The tele library is supposed already configured.
APPLETS_PATH = tele.tools.config['applets_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = tele.SUPERUSER_ID


def get_db_name():
    db = tele.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Tele module name.
    """
    def register(func):
        # register func by tele module name
        if func.__module__.startswith('tele.applets.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most tele operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class TeleSuite(unittest.suite.TestSuite):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from tele.http import root
        if not root._loaded:
            root.load_applets()
            root._loaded = True
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('tele.applets.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Tele-specific code. This class is abstract and
    ...
    longMessage = True      # more verbose error message by default: https://www.tele.studio/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~tele.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('tele.sql_db.Cursor.execute', execute):
            with patch('tele.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/tele/applets/" in filename:
                        filename = filename.rsplit("/tele/applets/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = tele.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, tele.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_tele')
        self.chrome_pid = None

        otc = tele.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Tele widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{tele.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True, head=False):
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, tele.tools.config['http_port'], url)
        if head:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('tele.service.http.request.')]
        ...
        if request_threads:
            ...
            tele.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        tele.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            tele.http.root.session_store.delete(self.session)

        self.session = session  = tele.http.root.session_store.new()
        session.db = db
        ...
        tele.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return "http://%s:%s" % (HOST, tele.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "tele.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "tele.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('tele.http.Request._get_profiler_context_manager', route_profiler))
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~tele.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~tele.fields.Many2many` fields, the field itself is a
    :class:`~tele.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~tele.fields.One2many` are reified as
    :class:`~tele.tests.common.O2MProxy`.

    Because the :class:`~tele.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~tele.tests.common.O2MProxy.new` and
    :meth:`~tele.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: tele.models.Model
    ...
    :type view: int | str | tele.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~tele.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~tele.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = tele.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = tele.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return tele.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return tele.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from tele.tests.common have a test_tags

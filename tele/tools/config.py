...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.telerc (or to ~/.telecms_serverrc if it exists)")
        ...
        group.add_option("--applets-path", dest="applets_path",
                         help="specify additional applets paths (separated by commas).",
                         action="callback", callback=self._check_applets_path, nargs=1, type="string")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'tele_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "tele.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="tele.http.rpc.request:DEBUG", help='shortcut for --log-handler=tele.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="tele.http.rpc.response:DEBUG", help='shortcut for --log-handler=tele.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="tele.http:DEBUG", help='shortcut for --log-handler=tele.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="tele.sql_db:DEBUG", help='shortcut for --log-handler=tele.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Tele to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes tele.tools.config and telecms.conf (the
        ...
        Typical usage of this method:
            ...
            tele.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        tele.netsvc.init_logger()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'tele.conf')
        else:
            rcfilepath = os.path.expanduser('~/.telerc')
            old_rcfilepath = os.path.expanduser('~/.telecms_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.telerc' and '.telecms_serverrc' in your path. Please keep only one of "\
                "them, preferably '.telerc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('TELE_RC') or os.environ.get('TELECMS_SERVER') or rcfilepath)
        self.load()
        ...
        keys = ['http_interface', 'http_port', 'longpolling_port', 'http_enable',
                ...
                'db_maxconn', 'import_partial', 'applets_path', 'upgrade_path',
        ...
        if not self.options['applets_path'] or self.options['applets_path']=='None':
            default_applets = []
            base_applets = os.path.join(self.options['root_path'], 'applets')
            if os.path.exists(base_applets):
                default_applets.append(base_applets)
            main_applets = os.path.abspath(os.path.join(self.options['root_path'], '../applets'))
            if os.path.exists(main_applets):
                default_applets.append(main_applets)
            self.options['applets_path'] = ','.join(default_applets)
        else:
            self.options['applets_path'] = ",".join(
                self._normalize(x)
                for x in self.options['applets_path'].split(','))
        ...
        conf.applets_paths = self.options['applets_path'].split(',')
    ...
    def _is_applets_path(self, path):
        from tele.modules.module import MANIFEST_NAMES
    ...
    def _check_applets_path(self, option, opt, value, parser):
        ad_paths = []
        for path in value.split(','):
            ...
            if not self._is_applets_path(res):
                raise optparse.OptionValueError("option %s: the path %r is not a valid applets directory" % (opt, path))
            ad_paths.append(res)
    ...
    @property
    def applets_data_dir(self):
        add_dir = os.path.join(self['data_dir'], 'applets')
        ...
        if not os.path.exists(d):
            try:
                ...
                logging.getLogger(__name__).debug('Failed to create applets data dir %s', d)
        return d

...
STANDARD_MODULES = ['web', 'tele_enterprise', 'theme_common', 'base']
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in tele.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        # Exclude standard applets paths
        exclude_heuristic = [tele.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(tele.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be tele/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = tele.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = tele.SUPERUSER_ID
            env = tele.api.Environment(cr, uid, {})
            self.count_env(env)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Tele cloc", lines="Line", other="Other", code="Code")
        s += hr

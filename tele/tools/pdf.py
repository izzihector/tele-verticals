...
from tele.tools.misc import file_open
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Tele's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Tele's logo in the banner.
    ...
    tele_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    tele_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(tele_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class TelePdfFileReader(PdfFileReader):
...
class TelePdfFileWriter(PdfFileWriter):
    ...
    def embed_tele_attachment(self, attachment, subtype=None):
        assert attachment, "embed_tele_attachment cannot be called without attachment."

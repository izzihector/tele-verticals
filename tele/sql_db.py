...
"""
The PostgreSQL connector is a connectivity layer between the Tele code and
...
def flush_env(cr, *, clear=True):
    warnings.warn("Since Tele 15.0, use cr.flush() instead of flush_env(cr).",
...
def clear_env(cr):
    warnings.warn("Since Tele 15.0, use cr.clear() instead of clear_env(cr).",
                  DeprecationWarning, stacklevel=2)
    cr.clear()
...
class Cursor(BaseCursor):
    """Represents an open transaction to the PostgreSQL DB backend,
       ...
       ``cursor`` objects.
        ...
        such as Tele, we need the best guarantees that no data
        ...
        Tele implements its own level of locking protection
        ...
        the default transaction isolation level for Tele cursors, as
    ...
    def autocommit(self, on):
        if on:
            warnings.warn(
                "Since Tele 13.0, the ORM delays UPDATE queries for "
...
def close_db(db_name):
    """ You might want to call tele.modules.registry.Registry.delete(db_name) along this function."""
    global _Pool
    if _Pool:

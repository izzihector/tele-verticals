...
import tele
from tele.modules import get_modules
from tele.modules.registry import Registry
from tele.release import nt_service_name
from tele.tools import config
from tele.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'tele.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The TELE_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("TELE_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by Tele.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(tele, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in tele.applets.__path__:
            _logger.info('Watching applets folder %s', path)
...
class FSWatcherInotify(FSWatcherBase):
    def __init__(self):
        self.started = False
        # ignore warnings from inotify in case we have duplicate applets paths.
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="tele.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from tele.applets.base.models.ir_cron import ir_cron
        conn = tele.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = tele.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(tele.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="tele.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="tele.service.httpd")
        t.daemon = True
        t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = tele.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('tele: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(tele.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The TELE_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("TELE_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = tele.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('tele.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from tele.applets import base
            base.models.ir_cron.ir_cron._process_jobs(db_name)

            # dont keep cursors in multi database mode
            if len(db_names) > 1:
                tele.sql_db.close_db(db_name)
            if rpc_request_flag:
    ...
    def start(self):
        ...
        dbconn = tele.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(tele.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            tele.modules.module.load_telecms_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the applets found in the `tele-web` project.
Maybe you forgot to add those applets in your applets_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute tele-server process with (nearly) the same arguments"""
    if tele.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from tele.tests.common import TeleSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = tele.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             tele.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the tele http server and cron processor.
    """
    global server
    ...
    if tele.evented:
        server = GeventServer(tele.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(tele.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Tele is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(tele.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not tele.evented:
        if inotify:

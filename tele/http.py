...
# *broken*
NO_POSTMORTEM = (tele.exceptions.AccessDenied,
                 tele.exceptions.UserError,
                 tele.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                tele.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = tele.service.common.dispatch
        elif service_name == 'db':
            dispatch = tele.service.db.dispatch
        elif service_name == 'object':
            dispatch = tele.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                tele.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                tele.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(tele.tools.exception_to_unicode(e))
        tele.tools.debugger.post_mortem(tele.tools.config, sys.exc_info())
        raise


class WebRequest(object):
    """ Parent class for all Tele Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in tele.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~tele.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~tele.api.Environment` bound to current request. """
        if self._env is None:
            self._env = tele.api.Environment(self.cr, self.uid, self.context)
        return self._env

    @lazy_property
    def session(self):
        """ :class:`TeleSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            tele.tools.debugger.post_mortem(
                tele.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Tele implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <tele.http.WebRequest.csrf_token`, the
          :data:`~tele.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Tele) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (tele.exceptions.UserError,
...
HTTP methods). See
https://www.tele.studio/documentation/15.0/developer/reference/applets/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["tele", "applets"]:
            module = ""
        else:
...
class EndPoint(object):
    ...
    # When Tele clears the routing map, new EndPoint objects are created,
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('tele.applets.') and c.__module__.split(".")[2] in modules
...
class TeleSession(sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(TeleSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = tele.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = tele.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ Tele provides languages which may not make sense and/or may not
...
TELE_DISABLE_SESSION_GC = str2bool(os.environ.get('TELE_DISABLE_SESSION_GC', '0'))

if TELE_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or tele.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the Tele Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def load_applets(self):
        """ Load all applets from applets path containing static files and
        controllers and configure them.  """
        # TODO should we move this to ir.http so that only configured modules are served ?
        statics = {}
        manifests = applets_manifest
        for applets_path in tele.applets.__path__:
            for module in sorted(os.listdir(str(applets_path))):
                if module not in manifests:
                    # Deal with the manifest first
                    mod_path = opj(applets_path, module)
                    manifest = read_manifest(applets_path, module)
                    if not manifest or (not manifest.get('installable', True) and 'assets' not in manifest):
                        continue
                    manifest['applets_path'] = applets_path
                    manifests[module] = manifest
                    # Then deal with the statics
                    path_static = opj(applets_path, module, 'static')
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-Tele-Session-Id")
        if not sid:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request_manager:
                ...
                if db:
                    try:
                        tele.registry(db).check_signaling()
                        with tele.tools.mute_logger('tele.sql_db'):
...
def db_list(force=False, httprequest=None):
    try:
        dbs = tele.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if tele.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = tele.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif tele.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Tele will
        # use the value of --database as a comma separated list of exposed databases.
        exposed_dbs = set(db.strip() for db in tele.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = tele.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = tele.tools.ustr(filename)

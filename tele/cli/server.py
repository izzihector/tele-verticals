...
"""
Tele - Server
Tele is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - Tele SA
"""
...
__author__ = tele.release.author
__version__ = tele.release.version

# Also use the `tele` logger for the main script.
_logger = logging.getLogger('tele')
...
def report_configuration():
    ...
    """
    config = tele.tools.config
    _logger.info("Tele version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('applets paths: %s', tele.applets.__path__)
...
def setup_pid_file():
    ...
    """
    config = tele.tools.config
    if not tele.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = tele.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            tele.tools.trans_export(config["language"],
...
def main(args):
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                tele.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except tele.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        tele.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = tele.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the tele server (default command)"""
    def run(self, args):
        main(args)

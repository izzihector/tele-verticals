...
# Models for client
# ----------------------------------------------------------
class IomtBox(models.Model):
    _name = 'iomt.box'
    _description = 'IoMT Box'

    name = fields.Char('Name', readonly=True)
    identifier = fields.Char(string='Identifier (Mac Address)', readonly=True)
    device_ids = fields.One2many('iomt.device', 'iomt_id', string="Devices", readonly=True)
    device_count = fields.Integer(compute='_compute_device_count')
    ip = fields.Char('Domain Address', readonly=True)
    ip_url = fields.Char('IoMT Box Home Page', readonly=True, compute='_compute_ip_url')
    drivers_auto_update = fields.Boolean('Automatic drivers update', help='Automatically update drivers when the IoMT Box boots', default=True)
...
class IomtDevice(models.Model):
    _name = 'iomt.device'
    _description = 'IOMT Device'

    iomt_id = fields.Many2one('iomt.box', string='IoMT Box', required=True, ondelete='cascade')
    ...
    iomt_ip = fields.Char(related="iomt_id.ip")
    company_id = fields.Many2one('res.company', 'Company', related="iomt_id.company_id")
    connected = fields.Boolean(string='Status', help='If device is connected to the IoMT Box', readonly=True)
    keyboard_layout = fields.Many2one('iomt.keyboard.layout', string='Keyboard Layout')
    ...
    def name_get(self):
        return [(i.id, "[" + i.iomt_id.name +"] " + i.name) for i in self]
